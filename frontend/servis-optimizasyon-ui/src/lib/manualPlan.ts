...
export function addManualStop(
  ...
  const stop: ScenarioStop = {
    id: stopId, location, assignedPersonIds: [], walkingDistancesMeters: {}, walkingDurationsSeconds: {},
    demand: 0, qualityScore: 1, averageWalkingDistanceMeters: 0,
  }
  const routes = plan.routes.some((route) => route.vehicleId === vehicleId)
    ? plan.routes.map((route) => route.vehicleId === vehicleId ? { ...route, geometry: '', distanceMeters: 0, durationSeconds: 0, stopIds: [...route.stopIds, stopId] } : route)
