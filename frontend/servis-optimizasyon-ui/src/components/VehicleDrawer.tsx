...
export function VehicleDrawer(props: VehicleDrawerProps) {
  ...
  const suggestedCapacity = ([18, 30, 46] as const).find((value) => value - reservedSeats >= personIds.length) ?? 46
      ...
      <div className="op-drawer-card">
        ...
        <div className="op-drawer-card-body op-scroll">
          <div className="op-vehicle-editor">
            ...
            <label>Fiziksel kapasite<select value={capacity} onChange={(event) => {
              const next = Number(event.target.value)
              setCapacity(next)
              if (reservedSeats >= next) setReservedSeats(next - 1)
            }}><option value={18}>18</option><option value={30}>30</option><option value={46}>46</option></select></label>
            <label>Boş bırakılacak koltuk<input type="number" min={0} max={capacity - 1} value={reservedSeats} onChange={(event) => setReservedSeats(Math.min(capacity - 1, Math.max(0, Number(event.target.value))))} /></label>
            <button className="op-btn op-btn-secondary" disabled={invalidCapacityChange} onClick={() => props.onUpdateVehicle({ plate: plate || null, capacity: capacity as ScenarioVehicle['capacity'], reservedSeats, effectiveCapacity })}>Araç ayarını kaydet</button>
      ...
      <div className="op-drawer-card">
        ...
        <div className="op-drawer-card-body op-scroll">
          <ul className="op-drawer-stop-list">
            <li><i style={{ background: color }} /><span><strong>Başlangıç</strong> · {vehicle?.start ? `${vehicle.start[1].toFixed(4)}, ${vehicle.start[0].toFixed(4)}` : 'ilk yolcu durağı'}</span></li>
            {routeStops.map((stop, index) => <li key={stop.id} className="op-stop-editor-row">
              <i className="op-dot-stop" />
              <span>{stop.id} · {stop.assignedPersonIds.length} kişi
                {unassignedPeople.length > 0 && <select defaultValue="" disabled={capacityReached} onChange={(event) => {
                  if (event.target.value) props.onAssignToStop(event.target.value, stop.id)
                  event.target.value = ''
                }}><option value="" disabled>{capacityReached ? 'Kapasite dolu' : '+ Yolcu ekle'}</option>{unassignedPeople.map((person) => <option value={person.id} key={person.id}>{person.name || person.id}</option>)}</select>}
              </span>
              <span className="op-order-buttons">
                <button disabled={index === 0} onClick={() => props.onMoveStop(stop.id, -1)}>↑</button>
                <button disabled={index === routeStops.length - 1} onClick={() => props.onMoveStop(stop.id, 1)}>↓</button>
              </span>
            </li>)}
            <li><i className="op-dot-workplace" /><span><strong>Varış</strong> · {workplace ? `${workplace[1].toFixed(4)}, ${workplace[0].toFixed(4)}` : '—'}</span></li>
          </ul>
        </div>
      </div>
    </div>
